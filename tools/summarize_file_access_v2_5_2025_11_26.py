...
CSV_HEADER = [
    "path", "touched_at_runtime", "reasons", "reachable_from_main", "is_src", "is_test", "is_archive", "is_gui", "category", "package"
]

def normalize_to_repo_relative(abs_path: Path, repo_root: Path, anchors=("src", "tests", "docs", "archive", "tools")):
    try:
...
def load_file_access_logs(log_dir: Path, repo_root: Path):
    ...
    print(f"[summarize] Loaded {total_lines} JSONL entries from {len(list(log_dir.glob('*.jsonl')))} files.")
    print(f"[summarize] Resolved {len(paths)} unique repo-relative paths.")
    print(f"[summarize] {len(unmatched_paths)} runtime paths could not be normalized (see unmatched list).")
    return paths
...
def categorize(path, inv):
    ...
    is_gui = inv.get("is_gui", False) or p.startswith("src/gui/")
...
def join_inventory_and_logs(inv_list, log_dict):
    ...
    for path in sorted(all_paths):
        ...
        row = {
            ...
            "reachable_from_main": str(inv.get("reachable_from_main", "")).lower() if "reachable_from_main" in inv else "",
            "is_src": str(path.startswith("src/")).lower(),
            "is_test": str(path.startswith("tests/") or inv.get("is_test", False)).lower(),
            "is_archive": str(path.startswith("archive/") or "(OLD" in path or "legacy" in path or "gui_v1_legacy" in path).lower(),
...
def write_markdown(rows, out_path):
    ...
    with out_path.open("w", encoding="utf-8") as f:
        f.write(f"# CLEANHOUSE File Access Report (V2.5)\n\n")
