"""Reference-only archived GUI V2 pipeline config panel."""
...
class PipelineConfigPanel(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        row = 0
        ttk.Label(self, text="Run Mode:", style=BODY_LABEL_STYLE).grid(
        ...
        ttk.Label(self, text="Batch Runs:", style=BODY_LABEL_STYLE).grid(
        ...
        batch_spin.bind("<FocusOut>", lambda _event: self._on_batch_change())
        row += 1

        ttk.Label(self, text="Randomizer:", style=BODY_LABEL_STYLE).grid(
            row=row, column=0, sticky="w", pady=2
        )
    ...
    def apply_run_config(self, config: dict[str, Any]) -> None:
        if "loop_count" in config.get("pipeline", {}):
            try:
                self.batch_var.set(max(1, int(config["pipeline"]["loop_count"])))
    ...
    def _on_batch_change(self) -> None:
        try:
            ...
            value = 1
        self.batch_var.set(max(1, value))
        if callable(self._on_change):
            self._on_change()
    ...
    def _refresh_lora_controls(self) -> None:
        ...
        if not entries:
            ttk.Label(
                self._lora_container,
                text="LoRA strengths not available",
        ...
        for entry in entries:
            ...
            ttk.Checkbutton(
                ...
                text="Enabled",
                variable=check_var,
                command=lambda lora_name=name, bool_var=check_var: self._on_lora_enabled_change(
                    lora_name, bool_var.get()
                ),
            ).pack(side="right")
    ...
    def get_randomizer_config(self) -> dict[str, Any]:
        try:
            ...
            max_variants = 1
        return {
            "randomization_enabled": bool(self.randomizer_enabled_var.get()),
