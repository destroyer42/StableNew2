"""Reference-only archived GUI V1 pipeline config panel."""
...
class PipelineConfigPanel(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.adetailer_var = tk.BooleanVar(
            value=getattr(self.pipeline_state, "stage_adetailer_enabled", True)
        )
        self.upscale_var = tk.BooleanVar(value=self.pipeline_state.stage_upscale_enabled)
        tk.Checkbutton(
            self,
            text="Enable txt2img",
            variable=self.txt2img_var,
    ...
    def _refresh_lora_controls(self) -> None:
        ...
        if not metadata.loras:
            ttk.Label(self._lora_frame, text="No LoRAs detected", foreground="gray").pack(
                anchor="w"
            )
            return
        ...
        for lora_ref in metadata.loras:
            ...
            checkbox = ttk.Checkbutton(
                ...
                command=lambda name=lora_name, var=enabled_var: self._on_lora_enabled_change(
                    name, var.get()
                ),
            )
            ...
            scale = ttk.Scale(
                ...
                orient="horizontal",
                command=lambda value, name=lora_name: self._on_lora_strength_change(
                    name, float(value)
                ),
            )
