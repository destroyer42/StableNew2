...
def test_global_prompt_integration(tmp_path: Path):
    ...
    print(
        f"   apply_global_positive_txt2img: {final_config['pipeline']['apply_global_positive_txt2img']}"
    )
    print(
        f"   apply_global_negative_txt2img: {final_config['pipeline']['apply_global_negative_txt2img']}"
    )

    # Verify config flags match checkbox state
    assert not final_config["pipeline"]["apply_global_positive_txt2img"], (
        "Positive should be disabled"
    )
    assert final_config["pipeline"]["apply_global_negative_txt2img"], (
        "Negative should be enabled"
    )
    ...
    try:
        with (
            patch(
                "src.pipeline.executor.save_image_from_base64", return_value=tmp_path / "image.png"
            ),
        ...
        assert not positive_calls[0], "Positive should NOT be applied"
        assert negative_calls[0], "Negative SHOULD be applied"
