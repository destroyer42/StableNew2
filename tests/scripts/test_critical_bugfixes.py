...
def test_reprocessing_guard_logic():
    ...
    assert processed == ["adetailer", "upscale"], (
        f"❌ Should process adetailer+upscale, got {processed}"
    )
    ...
    assert processed == ["img2img", "adetailer", "upscale"], (
        f"❌ Should process img2img+adetailer+upscale, got {processed}"
    )
