...
def tk_root():
    """Create and destroy a Tk root per test; skip if Tk is unavailable."""
...
def test_run_starts_pipeline_and_returns_to_idle(controller: AppController) -> None:
...
def test_second_run_after_first_completes_succeeds(controller: AppController) -> None:
    fake = controller._test_fake_runner  # type: ignore[attr-defined]
...
def test_stop_sets_cancel_and_updates_lifecycle(tk_root) -> None:
    window = MainWindow(tk_root)
...
def test_pipeline_error_sets_error_state_and_recovers(tk_root) -> None:
    window = MainWindow(tk_root)
