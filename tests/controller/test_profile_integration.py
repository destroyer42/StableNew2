...
def test_build_pipeline_config_with_profiles_applies_suggested_preset():
    ...
    with (
        patch(
            "src.learning.model_profiles.find_model_profile_for_checkpoint",
            return_value=MagicMock(),
        ),
        patch("src.learning.model_profiles.find_lora_profile_for_name", return_value=MagicMock()),
        patch(
            "src.learning.model_profiles.suggest_preset_for", return_value=make_suggested_preset()
        ),
    ):
        config = controller.build_pipeline_config_with_profiles(
            base_model_name="TestModel", lora_names=["TestLora"], user_overrides={}
        )
        ...
        assert any(
            lora["name"] == "TestLora" and lora["weight"] == 0.8
            for lora in config["txt2img"]["loras"]
        )
...
def test_build_pipeline_config_with_profiles_falls_back_without_profiles():
    controller = PipelineController()
    with (
        patch("src.learning.model_profiles.find_model_profile_for_checkpoint", return_value=None),
