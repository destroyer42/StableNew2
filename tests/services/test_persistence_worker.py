...
class TestPersistenceWorker:
    ...
    def test_enqueue_returns_immediately(self):
        ...
        from src.services.persistence_worker import PersistenceTask, PersistenceWorker
    ...
    def test_write_happens_on_worker_thread(self):
        ...
        try:
            ...
            assert write_thread_id != main_thread_id, (
                "Write should happen on worker thread, not main thread"
            )
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                for i in range(3):
                    task = PersistenceTask(
                        task_type="manifest",
                        data={
                            "file_path": str(Path(tmpdir) / f"test_{i}.json"),
                            "payload": {"i": i},
                        },
                    )
                    worker.enqueue(task)
