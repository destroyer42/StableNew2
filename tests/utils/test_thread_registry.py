...
class TestThreadRegistry:
    ...
    def test_spawn_creates_and_tracks_thread(self):
        ...
        thread = registry.spawn(target=worker, name="TestWorker", purpose="Test thread tracking")
    ...
    def test_spawn_with_args_and_kwargs(self):
        ...
        thread = registry.spawn(target=worker, args=(1, 2), kwargs={"z": 3}, name="TestArgsWorker")
    ...
    def test_spawn_warns_on_daemon_threads(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            thread = registry.spawn(target=worker, name="TestDaemonWorker", daemon=True)
    ...
    def test_get_active_threads_cleans_dead_threads(self):
        ...
        thread = registry.spawn(target=quick_worker, name="QuickWorker")
    ...
    def test_unregister_removes_thread(self):
        ...
        thread = registry.spawn(target=worker, name="UnregisterWorker")
    ...
    def test_shutdown_all_joins_non_daemon_threads(self):
        ...
        thread = registry.spawn(target=worker, name="ShutdownWorker", daemon=False)
    ...
    def test_shutdown_all_reports_timeout_threads(self):
        ...
        thread = registry.spawn(target=worker, name="TimeoutWorker", daemon=False)
    ...
    def test_shutdown_all_reports_orphaned_daemon_threads(self):
        ...
        thread = registry.spawn(target=worker, name="OrphanDaemonWorker", daemon=True)
    ...
    def test_dump_status_shows_thread_info(self):
        ...
        thread = registry.spawn(target=worker, name="StatusWorker", purpose="Testing status dump")
    ...
    def test_tracked_thread_dataclass(self):
        ...
        tracked = TrackedThread(
            thread=thread, name="TestThread", spawned_at=time.monotonic(), purpose="Test purpose"
        )
    ...
    def test_shutdown_all_threads_convenience_function(self):
        ...
        thread = registry.spawn(target=worker, name="ConvenienceWorker")
...
class TestThreadRegistryIntegration:
    ...
    def test_multiple_workers_shutdown_cleanly(self):
        ...
        for i in range(num_workers):
            thread = registry.spawn(target=worker, args=(i,), name=f"Worker-{i}", daemon=False)
    ...
    def test_shutdown_with_mixed_daemon_and_normal_threads(self):
        ...
        normal_thread = registry.spawn(target=worker, name="NormalThread", daemon=False)

        registry.spawn(target=worker, name="DaemonThread", daemon=True)
