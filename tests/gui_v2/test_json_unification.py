...
def test_unified_json_no_collision():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"  ✓ pack_data preserved: slots={len(data['pack_data']['slots'])}, matrix slots={len(data['pack_data']['matrix']['slots'])}"
        )
        ...
        print(
            f"  ✓ Loaded pack: name={loaded_pack.name}, slots={len(loaded_pack.slots)}, matrix enabled={loaded_pack.matrix.enabled}"
        )
