...
def test_pipeline_panel_stage_card_order():
    ...
    panel.sidebar = SimpleNamespace(get_enabled_stages=lambda: {"txt2img", "img2img", "adetailer", "upscale"})
    panel._apply_stage_visibility()

    # Ensure the card ordering matches canonical sequence
    body_children = panel.body.pack_slaves()
    order_map = {panel.txt2img_card: 0, panel.img2img_card: 1, panel.adetailer_card: 2, panel.upscale_card: 3}
