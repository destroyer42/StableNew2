...
def test_content_visibility_mode_journey_filters_live_and_persists(
    ...
    with patch("src.gui.main_window_v2.get_ui_state_store", return_value=store):
        ...
        try:
            ...
            assert first.window.header_zone.visibility_button.cget("text") == "Visibility: NSFW"

            first.window.header_zone.visibility_button.invoke()
            first_root.update()

            assert first.controller.app_state.content_visibility_mode == "sfw"
            assert first.window.header_zone.visibility_button.cget("text") == "Visibility: SFW"
            assert list(prompt_tab.pack_listbox.get(0, tk.END)) == ["safe_pack"]
            assert "prompt pack" in prompt_tab.visibility_notice_var.get().lower()
            ...
            assert "SFW mode active" in queue_panel.visibility_banner.cget("text")
            assert running_job_panel.visibility_banner.cget("text") == "SFW mode active"
        finally:
            first.cleanup()
            try:
                first_root.destroy()
        ...
        try:
            ...
            assert second.window.header_zone.visibility_button.cget("text") == "Visibility: SFW"
