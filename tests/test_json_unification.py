...
from src.gui.models.prompt_pack_model import MatrixConfig, MatrixSlot, PromptPackModel, PromptSlot
...
def test_unified_json_no_collision():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"  - pack_data.matrix.slots: {[s['name'] for s in data['pack_data']['matrix']['slots']]}"
        )
        ...
        print(
            f"  [OK] pack_data preserved: slots={len(data['pack_data']['slots'])}, matrix slots={len(data['pack_data']['matrix']['slots'])}"
        )
        ...
        print(
            f"  [OK] Loaded pack: name={loaded_pack.name}, slots={len(loaded_pack.slots)}, matrix enabled={loaded_pack.matrix.enabled}"
        )
