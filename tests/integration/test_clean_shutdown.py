...
class TestCleanShutdown:
    ...
    def test_no_orphan_threads_after_registry_shutdown(self):
        ...
        for i in range(3):
            ...
            registry.spawn(target=worker, name=f"TestWorker-{i}", daemon=False)
    ...
    def test_thread_count_matches_before_and_after(self):
        ...
        for i in range(5):
            registry.spawn(target=worker, name=f"CountTestWorker-{i}", daemon=False)
    ...
    def test_no_zombie_processes_after_shutdown(self):
        ...
        for i in range(3):
            registry.spawn(target=worker, name=f"ProcessTestWorker-{i}", daemon=False)
    ...
    def test_daemon_threads_do_not_prevent_shutdown(self):
        ...
        registry.spawn(
            target=slow_daemon_worker, name="SlowDaemonWorker", daemon=True
        )
    ...
    def test_shutdown_is_idempotent(self):
        ...
        registry.spawn(target=worker, name="IdempotentWorker", daemon=False)
...
class TestRealWorldShutdownScenarios:
    ...
    def test_shutdown_with_io_bound_threads(self):
        ...
        for i in range(3):
            registry.spawn(target=io_worker, name=f"IOWorker-{i}", daemon=False)
    ...
    def test_shutdown_with_cpu_bound_threads(self):
        ...
        for i in range(2):
            registry.spawn(target=cpu_worker, name=f"CPUWorker-{i}", daemon=False)
    ...
    def test_shutdown_with_exception_in_thread(self):
        ...
        thread = registry.spawn(target=crashing_worker, name="CrashingWorker", daemon=False)
