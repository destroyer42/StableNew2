...
| Operator readiness | Open | Readiness projection/UI and runtime hardening are integrated; the Windows runtime/bootstrap baseline, real local native SVD XT square-source run through the public production runner, replay lineage, separate artifact generation, cancellation, and real portrait `832x1216 -> 640x960` source-aware production acceptance are accepted. Remaining open work is RIFE semantics if required, portable video provenance and parent artifact lineage, queue/history metadata and recovery UX, and final operator-facing closeout | `PR-MVP-080` |
