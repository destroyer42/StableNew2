...
_LORA_NAME_PATTERN = re.compile(r"<\s*lora\s*:([^:>]+)(?::[^>]+)?>", re.IGNORECASE)
...
def build_dedupe_key(text: str) -> str:
    ...
    if detect_lora_syntax(normalized):
        match = _LORA_NAME_PATTERN.search(normalized)
        return f"lora:{match.group(1).strip()}" if match else normalized
