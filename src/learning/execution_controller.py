...
class LearningExecutionController:
    ...
    def __init__(
        ...
        _logger.info(
            f"[LearningExecutionController]   learning_state: {learning_state is not None}"
        )
        ...
        _logger.info(
            "[LearningExecutionController] Checking job_service for callback registration..."
        )
        _logger.info(
            f"[LearningExecutionController]   job_service is None: {self.job_service is None}"
        )
        if self.job_service:
            _logger.info(
                f"[LearningExecutionController]   hasattr register_callback: {hasattr(self.job_service, 'register_callback')}"
            )
        ...
        if self.job_service and hasattr(self.job_service, "register_callback"):
            ...
            _logger.warning(
                "[LearningExecutionController] ⚠ Could NOT register callbacks - job_service unavailable or missing method"
            )
    ...
    def _handle_job_finished(self, job: Any) -> None:
        ...
        image_paths = []
        _logger.info(
            f"[ExecutionController] Processing job {job_id}, found {len(variants)} variants in result"
        )
        for idx, variant in enumerate(variants):
            if isinstance(variant, dict):
                ...
                if paths:
                    ...
                    _logger.info(
                        f"[ExecutionController] Variant {idx} yielded {len(paths)} artifact path(s)"
                    )
                else:
                    _logger.warning(
                        f"[ExecutionController] Variant {idx} has no recognized path keys"
                    )
    ...
    def _handle_job_failed(self, job: Any, error: Any = None) -> None:
        ...
        _logger.info(
            f"[LearningExecutionController] Job failed notification: {job_id} - {error_msg}"
        )
    ...
    def on_job_completed(self, job_id: str, result: dict[str, Any]) -> None:
        ...
        _logger.info(
            f"Learning job completed: variant={variant.param_value}, images={len(image_refs)}"
        )
