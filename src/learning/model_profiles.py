...
class ModelProfile:
    ...
    default_refiner_id: str | None = None
    default_hires_upscaler_id: str | None = None
    default_hires_denoise: float | None = None
...
# --- Sidecar Loaders ---
def load_model_profile(path: Path) -> ModelProfile | None:
    if not path.exists():
        return None
    try:
...
def load_lora_profile(path: Path) -> LoraProfile | None:
    if not path.exists():
        return None
    try:
        with open(path, encoding="utf-8") as f:
...
def find_lora_profile_for_name(
    lora_name: str, lora_search_paths: Sequence[Path]
) -> LoraProfile | None:
...
def suggest_preset_for(
    model_profile: ModelProfile | None, lora_profiles: Sequence[LoraProfile]
    ...
    presets = sorted(
        model_profile.recommended_presets, key=lambda p: rating_order.get(p.rating, 0), reverse=True
    )
    chosen = presets[0]
    # Merge LoRA weights
    lora_weights = {}
