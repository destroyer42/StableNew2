...
class PromptSection(ttk.Frame):
    def __init__(self, master: tk.Misc, *, title: str = "Prompt", height: int = 3, **kwargs) -> None:
        super().__init__(master, style="Panel.TFrame", padding=4, **kwargs)
        ttk.Label(self, text=title, style="Muted.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 2))
        self.text = tk.Text(
            self,
            height=height,
            wrap="word",
...
class SamplerSection(ttk.Frame):
    def __init__(self, master: tk.Misc, *, title: str = "Sampler / Steps / CFG", **kwargs) -> None:
        super().__init__(master, style="Panel.TFrame", padding=4, **kwargs)
        ttk.Label(self, text=title, style="Muted.TLabel").grid(row=0, column=0, columnspan=4, sticky="w", pady=(0, 4))

        self.sampler_var = tk.StringVar()
        self.steps_var = tk.StringVar(value="20")
        self.cfg_var = tk.StringVar(value="7.0")

        ttk.Label(self, text="Sampler", style="Muted.TLabel").grid(row=1, column=0, sticky="w", padx=(0, 4))
        ttk.Entry(self, textvariable=self.sampler_var, width=18).grid(row=1, column=1, sticky="ew", padx=(0, 8))

        ttk.Label(self, text="Steps", style="Muted.TLabel").grid(row=1, column=2, sticky="w", padx=(0, 4))
        ttk.Entry(self, textvariable=self.steps_var, width=8).grid(row=1, column=3, sticky="ew")

        ttk.Label(self, text="CFG", style="Muted.TLabel").grid(row=2, column=0, sticky="w", padx=(0, 4), pady=(4, 0))
...
class SeedSection(ttk.Frame):
    def __init__(self, master: tk.Misc, *, title: str = "Seed", **kwargs) -> None:
        ...
        ttk.Checkbutton(self, text="Randomize", variable=self.randomize_var, style="Secondary.TButton").grid(
