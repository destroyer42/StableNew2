...
class PipelineConfigPanel(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc, pipeline_state: PipelineState, app_state: Any = None, on_change: Callable[[], None] | None = None, *args, **kwargs) -> None:
        ...
        tk.Checkbutton(self, text="Enable txt2img", variable=self.txt2img_var, command=self._on_stage_change).pack(anchor="w")
        tk.Checkbutton(self, text="Enable img2img", variable=self.img2img_var, command=self._on_stage_change).pack(anchor="w")
        tk.Checkbutton(self, text="Enable ADetailer", variable=self.adetailer_var, command=self._on_stage_change).pack(anchor="w")
        tk.Checkbutton(self, text="Enable upscale", variable=self.upscale_var, command=self._on_stage_change).pack(anchor="w", pady=(0, 6))
