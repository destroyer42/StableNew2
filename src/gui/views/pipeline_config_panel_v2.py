...
class PipelineConfigPanel(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc, pipeline_state: PipelineState, app_state: Any = None, on_change: Callable[[], None] | None = None, *args: Any, **kwargs: Any) -> None:
    ...
    def _refresh_lora_controls(self) -> None:
        ...
        for lora_ref in metadata.loras:
            ...
            def lora_enabled_command(name: str = lora_name, var: tk.BooleanVar = enabled_var) -> None:
                self._on_lora_enabled_change(name, var.get())
            checkbox = ttk.Checkbutton(
                lora_frame,
                text=lora_name,
                variable=enabled_var,
                command=lora_enabled_command
            )
            checkbox.pack(side="left")

            # Scale for strength
            strength_var = tk.DoubleVar(value=settings.strength)
            def lora_strength_command(val: str, name: str = lora_name) -> object:
                try:
                    float_val = float(val)
                except Exception:
                    float_val = 0.0
                self._on_lora_strength_change(name, float_val)
                return None
            scale = ttk.Scale(
                lora_frame,
                from_=0.0,
...
PipelineConfigPanel = PipelineConfigPanel
