...
class ConfigSweepWidgetV2(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        header_frame = ttk.Frame(
            self, style=getattr(self.theme, "SURFACE_FRAME_STYLE", theme_mod.SURFACE_FRAME_STYLE)
        )
        ...
        ttk.Checkbutton(apply_frame, text="txt2img", variable=self.apply_txt2img_var).grid(
            row=1, column=0, sticky="w"
        )
        ttk.Checkbutton(apply_frame, text="img2img", variable=self.apply_img2img_var).grid(
            row=1, column=1, sticky="w", padx=(8, 0)
        )
        ttk.Checkbutton(apply_frame, text="upscale", variable=self.apply_upscale_var).grid(
            row=2, column=0, sticky="w"
        )
        ttk.Checkbutton(apply_frame, text="adetailer", variable=self.apply_adetailer_var).grid(
    ...
    def _load_global_negative(self) -> None:
        ...
        if not self.config_manager or not hasattr(
            self.config_manager, "get_global_negative_prompt"
        ):
            return
