...
class LoRAKeywordDialog(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            header_frame, text=f"LoRA: {self.metadata.name}", font=("Segoe UI", 12, "bold")
        ).pack(anchor="w")

        if self.metadata.source != "none":
            ttk.Label(
                header_frame, text=f"Source: {self.metadata.source}", font=("Segoe UI", 9)
            ).pack(anchor="w")
        ...
        if self.metadata.keywords:
            ttk.Label(content_frame, text="Detected Keywords:", font=("Segoe UI", 10, "bold")).pack(
                anchor="w", pady=(0, 5)
            )
            ...
            ttk.Label(
                content_frame, text="No keywords detected", font=("Segoe UI", 10), foreground="gray"
        ...
        if self.metadata.description:
            ...
            ttk.Label(content_frame, text="Description:", font=("Segoe UI", 9, "bold")).pack(
                anchor="w"
            )
        ...
        if self.metadata.keywords:
            ttk.Button(button_frame, text="Copy All Keywords", command=self._copy_all).pack(
                side="left", padx=(0, 5)
            )
    ...
    def _filter_keywords(self) -> None:
        ...
        self.filtered_keywords = [kw for kw in self.metadata.keywords if search_text in kw.lower()]
        self._populate_keywords()
    ...
    def _add_keyword_entry(self, parent: ttk.Frame, keyword: str) -> None:
        ...
        ttk.Button(entry_frame, text="Copy", command=copy_keyword, width=8).pack(
            side="right", padx=(5, 0)
        )
