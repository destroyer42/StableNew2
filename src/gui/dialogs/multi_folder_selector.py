...
class MultiFolderSelector(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        self.add_btn = ttk.Button(
            btn_frame, text="Add Folder...", command=self._add_folder, width=15
        )
        self.add_btn.pack(side="left", padx=(0, 5))

        self.remove_btn = ttk.Button(
            btn_frame, text="Remove Selected", command=self._remove_selected, width=15
        )
        self.remove_btn.pack(side="left", padx=(0, 5))

        self.clear_btn = ttk.Button(btn_frame, text="Clear All", command=self._clear_all, width=15)
        self.clear_btn.pack(side="left", padx=(0, 5))

        # Status label
        self.status_label = ttk.Label(main_frame, text="0 folders selected", foreground="gray")
        ...
        cancel_btn = ttk.Button(action_frame, text="Cancel", command=self._on_cancel, width=12)
        cancel_btn.pack(side="right", padx=(5, 0))

        ok_btn = ttk.Button(action_frame, text="OK", command=self._on_ok, width=12)
    ...
    def _add_folder(self):
        ...
        """
        folder = filedialog.askdirectory(parent=self, title="Select Folder to Add")
