...
class EmbeddingScanner:
        ...
        for root, _dirs, files in os.walk(embeddings_dir):
            for file in files:
                if any(file.lower().endswith(ext) for ext in extensions):
                    ...
                    resources.append(
                        EmbeddingResource(name=name, path=str(file_path), file_size=file_size)
                    )
    ...
    def search_embeddings(self, query: str) -> list[EmbeddingResource]:
        ...
        return [emb for emb in self._embeddings if query_lower in emb.name.lower()]
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            data = [
                {"name": emb.name, "path": emb.path, "file_size": emb.file_size}
                for emb in self._embeddings
            ]
