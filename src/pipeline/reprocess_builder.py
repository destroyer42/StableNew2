...
class ReprocessJobBuilder:
    ...
    def build_reprocess_job(
        ...
        stages: list[
            Literal[
                "img2img", "adetailer", "upscale", "animatediff", "svd_native", "video_workflow"
            ]
        ],
        ...
        if invalid_stages:
            raise ValueError(
                f"Invalid reprocess stages: {invalid_stages}. Valid: {self.VALID_REPROCESS_STAGES}"
            )
        if not stages:
        ...
        for stage_name in stages:
            ...
            stage_config = StageConfig(
                ...
                denoising_strength=self._resolve_stage_denoise(
                    stage_name, config, stage_extra, defaults
                ),
        ...
        metadata = self._merge_nested_dicts(base_metadata, extra_metadata or {})
        video_stage_names = {"animatediff", "svd_native", "video_workflow"}
        is_video = bool(set(stages) & video_stage_names)
        source_aliases = {
        ...
        source_kind = (
            SourceKind.VIDEO_WORKFLOW
            if is_video
            else source_aliases.get(str(source).lower(), SourceKind.REPROCESS)
        )
        workload_type = WorkloadKind.VIDEO if is_video else WorkloadKind.IMAGE
        workload_common = {
            "positive_prompt": prompt,
            "negative_prompt": negative_prompt,
            "config": config,
            "input_image_paths": tuple(path_strs),
        ...
        workload = (
            VideoWorkloadSpec(
                **workload_common,
                sequence_intent=metadata.get("sequence_intent"),
    ...
    def build_effective_settings_preview(
        ...
        *,
        ...
        return ReprocessEffectiveSettingsPreview(
            ...
            negative_prompt_behavior=self.describe_prompt_behavior(
                negative_prompt_mode, negative_prompt_delta
            ),
                        ...
                        "source_items": [
                            {
                                ...
                                "image_edit": item.image_edit.to_dict()
                                if item.image_edit
                                else None,
                            }
                            for item in chunk
