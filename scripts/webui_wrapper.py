...
def main():
    ...
    if not SingleInstanceLock.is_gui_running():
        print(
            "[WebUI Wrapper] StableNew GUI is not running. Refusing to start WebUI.",
            file=sys.stderr,
        )
    ...
    while True:
        ...
        if time.time() - last_check >= check_interval:
            if not SingleInstanceLock.is_gui_running():
                print(
                    "[WebUI Wrapper] StableNew GUI has exited. Terminating WebUI.", file=sys.stderr
                )
                process.terminate()
                try:
                    ...
                    print(
                        "[WebUI Wrapper] WebUI did not terminate gracefully, killing...",
                        file=sys.stderr,
                    )
                    process.kill()
                sys.exit(1)
